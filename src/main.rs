...
const JDROCKS: (&[u8; 3], &[u8; 3]) = (b"VcS", b"j4:");

// Flags:
const PLI_PROTECTION: u32 = 0x00000002;
const ENCODING: u32 = 0x00000004;
...
struct VCS {
    ...
    version: u16,      // @ 0x0a "spEarth"; 0 or 1
    license_code: u32, // @ 0x10 "LibLicenseCode"

    /// 64-bit feedback block before `dewars_rocks` mixing
    last_block: [u8; 8], // @ 0x14
...
impl VCS {
    ...
    fn _vcs_sp_encode_on(&mut self) -> Vec<u8> {
        ...
        self.buffer[3] = b'G';
        self.buffer[4] = if self.flags & PLI_PROTECTION == PLI_PROTECTION {
            1
        } else {
            0
        };
        LittleEndian::write_u32(&mut self.buffer[0x1f..0x1f + 4], self.license_code);
    ...
    fn vodka_twist(buffer: &[u8], mut in_length: usize) -> (Vec<u8>, usize, bool) {
        let mut input = Vec::new();
        input.extend_from_slice(&buffer);
...
fn main() {
    ...
    let (mut decoded, _length, _more) = VCS::vodka_twist(&buffer[..], buffer.len());

    // println!("decoded: {:#x?}", decoded);
    // println!("length:  {:#x?}", _length);
    // println!("more:    {}", _more);
    ...
    let mut state = [0; 7 * 8 + 4];
    let mut i = 0xb;
    for o in 0..(state.len() / 7) + 1 {
        let o = o * 7;

        state[o] = decoded[0x10 + i];
        i += 1;
        if i == 0xf {
            i = 0x0;
        }

        state[o + 1] = decoded[0x10 + i];
        i += 1;
        if i == 0xf {
            i = 0x0;
        }

        state[o + 2] = decoded[0x10 + i];
        i += 1;
        if i == 0xf {
            i = 0x0;
        }

        state[o + 3] = decoded[0x10 + i];
        i = (i + 0x9) % 0xf;
        if i != 0xc {
            state[o + 4] = decoded[0x10 + i];
            state[o + 5] = decoded[0x11 + i];
            state[o + 6] = decoded[0x12 + i];
    ...
    // Decipher
    VCS::dewars_rocks(&state, &mut decoded[0..8]);
    VCS::dewars_rocks(&state, &mut decoded[0x1f..0x1f + 8]);

    // println!("deciphered: {:#x?}", decoded);
    ...
    let magic: String = decoded.iter().take(3).map(make_print).collect();
    ...
    let protected = if decoded[4] & 1 == 1 { "yes" } else { "no" };
    let last_block: String = decoded.iter().skip(0x8).take(0x8).map(make_print).collect();
    println!("magic:        {}", magic);
    println!("version:      {}", version);
    println!("G:            {}", make_print(&decoded[3]));
    println!("protected:    {}", protected);
    println!("last_block:   {}", last_block);
    println!("licence_code: {:?}", &decoded[0x1f..0x1f + 4]);

    // Now the fun begins!

    let buffer = br"E9TGFZb)R]R04^7[]PZH9Q2JA@SB+4\3-GE#78?R+GSgAQ90M9DZSDG860Y(4_:Y";
    let (decoded, _length, _more) = VCS::vodka_twist(&buffer[..], buffer.len());
    let s: String = decoded.iter().map(make_print).collect();
    println!("decoded:      {}", s);

    let mut vcs = VCS::new();
    vcs.length = 0x30;
    vcs.last_block.copy_from_slice(&decoded[0x8..0x10]);
    vcs.state.copy_from_slice(&state);
    vcs.buffer.copy_from_slice(&decoded);
    vcs.decipher();

    let deciphered: String = vcs.buffer.iter().map(make_print).collect();
    println!("deciphered:   {}", deciphered);
    println!("raw:          {:#x?}", &vcs.buffer[..]);
}
